...
class VersionedAPIData(object):
    def __init__(self,
        ...
        if single_value and primary_keys is not None:
            raise ValueError('single_value and primary_keys are mutually exclusive')
...
#    attributes which can have a `!` ahead should have `can_disable=True`
# 3. All bold attributes go into the `primary_keys` list -- this is not always true!


# ip/dhcp-server/; ip/firewall/; ip/ipsec/ path and subpaths have more can_disable=True set than what is shown in routeros 7.20.6 (on rb960pgs)
PATHS = {
    ...
    ('caps-man', 'access-list'): APIData(
        unversioned=VersionedAPIData(
            fully_understood=True,
            versioned_fields=[
                ([('7.15', '>=')], 'copy-from', KeyInfo()),
                ([('7.15', '>=')], 'place-before', KeyInfo()),
            ],
            fields={
    ...
    ('interface', 'vpls'): APIData(
        versioned=[
            ('7.15', '>=', VersionedAPIData(
                fully_understood=True,
                versioned_fields=[
                    ([('7.17', '>=')], 'bridge-pvid', KeyInfo(can_disable=True)),
                ],
                fields={
                    'arp': KeyInfo(),
                    'arp-timeout': KeyInfo(),
                    ...
                    'name': KeyInfo(),
                    'peer': KeyInfo(),
                    'pw-control-word': KeyInfo(can_disable=True),
                    'pw-l2mtu': KeyInfo(can_disable=True),
                    'pw-type': KeyInfo(can_disable=True),
    ...
    ('interface', 'wireless'): APIData(
        unversioned=VersionedAPIData(
            ...
            versioned_fields=[
                ...
                ([('7.15', '<')], 'running', KeyInfo(read_only=True)),
    ...
    ('user',): APIData(
        unversioned=VersionedAPIData(
            ...
            versioned_fields=[
                ([('7.15', '>=')], 'copy-from', KeyInfo()),
                ([('7.15', '<')], 'expired', KeyInfo(read_only=True)),
                ([('7.16', '>=')], 'inactivity-policy', KeyInfo()),
                ([('7.16', '>=')], 'inactivity-timeout', KeyInfo()),
                ([('7.15', '<')], 'last-logged-in', KeyInfo(read_only=True)),
            ],
            fields={
                'address': KeyInfo(),
                'comment': KeyInfo(can_disable=True, remove_value=''),
